...
async function whatsAsena () {
    ...
    setInterval(async () => { 
        ...
        await axios.get('https://gist.github.com/darkhacker2005/6fe4beab431fc7643f73543753c99794/row/').then(async (ann) => {
    ...
    conn.on('connecting', async () => {
        console.log(`${chalk.green.bold('DARK')}${chalk.blue.bold('KNIGHT')}
${chalk.white.bold('Version:')} ${chalk.red.bold('1.0')}
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                               ...
                               if (error.message.includes('URL')) {
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*🙇🏻කණගාටුයි, මට මෙම දෝෂය කියවීමට නොහැකි විය! 🙇🏻*' +
                                        '\n_වැඩි විස්තර සඳහා ඔබට අපගේ සහායක කණ්ඩායමට ලිවිය හැකිය.https://chat.whatsapp.com/KUSz1mWuSla2Li43UXbYiL _'
                                        , MessageType.text
                                    );
                                }    
                            }                      
